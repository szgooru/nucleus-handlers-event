...
public class AJUserRepo implements UserRepo {
  ...
  public JsonObject getUser(String userID) {
    ...
    User result = User.findById(userID);
    LOGGER.debug("AJUserRepo : getUser : findById : " + result);
    
    JsonObject returnValue = null;
    String[] attributes =  {"id", "firstname", "lastname", "email", "parent_user_id", "user_category",
                            "created_at", "updated_at", "last_login", "birth_date", "grade",
  ...
  private void addSchoolDistrictStateCountryInfo(JsonObject jsonToModify) {
    ...
    if ((schoolId != null) && !schoolId.isEmpty() ) {
      List<Map> schools = Base.findAll("select name from school where id = ?", schoolId);
    ...
    if ((schoolDistrictId != null) && !schoolDistrictId.isEmpty() ) {
      List<Map> schoolDists = Base.findAll("select name from school_district where id = ?", schoolDistrictId);
    ...
    if ((schoolStateId != null) && !schoolStateId.isEmpty() ) {
      List<Map> states = Base.findAll("select name from state where state_id = ?", schoolStateId);
    ...
    if ((countryId != null) && !countryId.isEmpty() ) {
      List<Map> countries = Base.findAll("select name from country where id = ?", countryId);
